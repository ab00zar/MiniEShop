RSpec.describe Carts::CartItemFactory do
  describe '#call' do
    let(:query) { [[2, 'A'], [3, 'B']] }
    let(:product_a) { instance_double(Product, code: 'A', price: 10.0) }
    let(:product_b) { instance_double(Product, code: 'B', price: 20.0) }
    ...
    before do
      allow(product_finder).to receive(:call).with(code: 'A').and_return(product_a)
      allow(product_finder).to receive(:call).with(code: 'B').and_return(product_b)
      allow(product_finder).to receive(:call).with(code: 'C').and_return(nil)
